...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid") 
	...
	auto onAxleAngularVelocityRequest{[&singleTrackModel, &INPUT_ID](cluon::data::Envelope &&envelope) {
		uint32_t const senderStamp = envelope.senderStamp();
		auto velocity = cluon::extractMessage<opendlv::proxy::AxleAngularVelocityRequest >(
                std::move(envelope));
		if (senderStamp == 0){
			singleTrackModel.setLeftAxleAngularVelocity(velocity);
		}
		else if (senderStamp == 1){
			singleTrackModel.setRightAxleAngularVelocity(velocity);
		}
}};  
      
    cluon::OD4Session od4{CID};
    od4.dataTrigger(opendlv::proxy::AxleAngularVelocityRequest ::ID(),
        onAxleAngularVelocityRequest);
